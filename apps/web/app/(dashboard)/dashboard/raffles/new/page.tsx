    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white dark:bg-slate-900 rounded-xl border p-6 space-y-4">
        <div><label className="block text-sm font-medium mb-1">Titulo</label>
          <input {...register("title")} className="w-full px-4 py-3 rounded-xl border dark:border-slate-700 dark:bg-slate-800" placeholder="Ej: Rifa Moto 0km" required /></div>
        <div><label className="block text-sm font-medium mb-1">Premio</label>
          <input {...register("prize")} className="w-full px-4 py-3 rounded-xl border dark:border-slate-700 dark:bg-slate-800" placeholder="Ej: Moto Yamaha" required /></div>
        <div className="grid grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium mb-1">Valor del premio</label>
            <input type="number" {...register("prizeValue")} className="w-full px-4 py-3 rounded-xl border dark:border-slate-700 dark:bg-slate-800" placeholder="15000000" required /></div>
          <div><label className="block text-sm font-medium mb-1">Total numeros</label>
            <input type="number" {...register("totalNumbers")} className="w-full px-4 py-3 rounded-xl border dark:border-slate-700 dark:bg-slate-800" placeholder="1000" required /></div>
        </div>
        <div><label className="block text-sm font-medium mb-1">Precio por numero</label>
          <input type="number" {...register("pricePerNumber")} className="w-full px-4 py-3 rounded-xl border dark:border-slate-700 dark:bg-slate-800" placeholder="10000" required /></div>
